...
\begin{tikzpicture}
  ...
  \node (middleReceiv1) at (\leftAlign + \loadingTime + 1.5*\sendingTime,1*\vspace){};
  \node (middleReceiv2) at (\leftAlign + \loadingTime + 2.5*\sendingTime,2*\vspace){};
  \node (middleReceiv3) at (\leftAlign + \loadingTime + 3.5*\sendingTime,3*\vspace){};
  ...
  % critical path
  \draw [->,thick] (startLoading.center) -- (middleReceivFinal0.center);
